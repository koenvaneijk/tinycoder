...
class Response:
    ...
    def raise_for_status(self) -> None:
        ...
        if 400 <= self.status_code < 600:
            reason = self.reason or f"Client Error {self.status_code}" if 400 <= self.status_code < 500 else f"Server Error {self.status_code}"
            raise HTTPError(f"{self.status_code} {reason} for url: {self.url}", response=self)

    def iter_content(self, chunk_size: int = 1, decode_unicode: bool = False) -> Generator[Union[bytes, str], None, None]:
    ...
    def iter_lines(self, chunk_size: int = 512, decode_unicode: bool = False, delimiter: Optional[Union[str, bytes]] = None) -> Generator[Union[bytes, str], None, None]:
...
def request(
    ...
    **kwargs: Any # Allow other requests kwargs, though they might not be used
    ...
    try:
        ...
        resp.headers = http_response.headers # This is an HTTPMessage object, acts like a dict
        ...
        if stream:
            ...
            if content_encoding == 'gzip':
                try:
                    ...
                    logger.debug("Decompressed gzipped response content.") # Add logger if available or use print
                except gzip.BadGzipFile:
                    logger.error("Failed to decompress gzip content, using raw content.") # Add logger
                    resp._content = raw_content # Fallback to raw content on error
                except Exception as e:
                    logger.error(f"Unexpected error during gzip decompression: {e}", exc_info=True) # Add logger
        ...
        try:
            ...
            if content_encoding == 'gzip':
                 try:
                     ...
                     logger.debug("Decompressed gzipped error response content.") # Add logger
                 except gzip.BadGzipFile:
                     logger.error("Failed to decompress gzipped error content, using raw content.") # Add logger
                     resp._content = raw_error_content # Fallback
                 except Exception as ex:
                     logger.error(f"Unexpected error during gzip decompression of error body: {ex}", exc_info=True) # Add logger
                     resp._content = raw_error_content # Fallback
            else:
            ...
            logger.error(f"Could not read error response body: {read_err}") # Add logger
            resp._content = None
        finally:
             resp._content_consumed = True
