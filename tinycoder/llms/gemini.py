...
class GeminiClient(LLMClient): # Inherit from LLMClient
    ...
    def __init__(self, model: Optional[str] = None, api_key: Optional[str] = None):
        ...
        print(f"INFO: Using Gemini model: {self.model}", file=sys.stderr) # Info message
    ...
    def generate_content(self, system_prompt: str, history: List[Dict[str, str]]) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            # print(f"DEBUG: Sending to Gemini URL: {self.api_url}") # Debugging
            # print(f"DEBUG: Sending Payload: {json.dumps(payload, indent=2)}") # Debugging
            # Make the POST request using the (potentially local) requests module
            response = requests.post(self.api_url, headers=headers, json=payload, timeout=180) # Longer timeout
            response.raise_for_status()  # Raise HTTPError for bad responses (4xx or 5xx)

            response_data = response.json()
            # print(f"DEBUG: Received from Gemini: {json.dumps(response_data, indent=2)}") # Debugging
