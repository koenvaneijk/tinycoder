...
YieldType = Dict[str, Union[Optional[str], Optional[List[Dict[str, Any]]], Optional[str]]] # {"type": "content/tool_call/error/finish", "data": ...}
...
def answer(
    ...
    tool_choice: Optional[Union[str, Dict]] = "auto", # Default to auto if tools are provided, API defaults to none otherwise
    ...
    if not api_key:
         ...
         return "[ERROR: API Key Missing]" if not stream else iter([{"type": "error", "data": "API Key Missing"}])
    ...
    if stream:
                            ...
                            try:
                                ...
                                if choices and len(choices) > 0:
                                    ...
                                    if tool_calls_delta:
                                        ...
                                        yield {"type": "tool_calls", "data": tool_calls_delta}
                                ...
                                logger.error(f"Error decoding JSON from stream: {data_str}")
                                yield {"type": "error", "data": f"JSON decode error: {data_str}"}
                            except Exception as e:
                                logger.error(f"Error processing stream chunk: {e} - Chunk: {data_str}")
                                yield {"type": "error", "data": f"Chunk processing error: {e}"}
        ...
        try:
            response = requests.post(
                API_URL, headers=headers, json=payload, stream=False # stream=False here
            )
            ...
            if choices and len(choices) > 0:
                ...
                if content:
                    logger.info(f"Non-streaming call successful. Finish reason: {choices[0].get('finish_reason')}")
                    return content # Return the full content string
                else:
                    # Handle cases where content might be missing (e.g., only tool calls)
                    logger.warning("Non-streaming response received, but content was empty.")
                    # Decide what to return here. Empty string? Raise error?
                    # For now, return empty string as the request was for "string response".
                    return ""
            else:
                logger.error(f"Non-streaming API call failed: No choices found in response. Response: {response_json}")
...
class DeepSeekClient(LLMClient):
    ...
    def __init__(self, model: Optional[str] = None, api_key: Optional[str] = None):
        ...
        super().__init__(model=resolved_model, api_key=resolved_api_key) # Pass resolved values to base
        print(f"INFO: Using DeepSeek model: {self.model}", file=sys.stderr) # Info message
    ...
    def generate_content(self, system_prompt: str, history: List[Dict[str, str]]) -> Tuple[Optional[str], Optional[str]]:
        """
